 ...
 * as it is.
 * Le coupe FT_Utils.h et FT_Utils.cpp utilise un pattern de programmation
 * appele Liaison dynamique avec repli statique.
 * L'objectif est de pouvoir compiler le même code sur plusieurs systèmes (Windows, Linux, Android). 
 * Sous Windows, charger un fichier .dll au moment de l'exécution, tandis que sous Android, lier 
 * la bibliothèque directement (statiquement).
 * 
*/
...
// Definition d'alias de pointeur sur fonctions
...
// Variables globales externes (pointeurs) qui contiendront les adresses réelles
// de fonctions une fois la bibliothèque chargée
...
extern "C"
{
	...
	void FT_Glyph_Get_CBox      ( FT_Glyph glyph, FT_UInt bbox_mode, FT_BBox* acbox);
 	...
 	FT_Error FTC_Manager_New    ( FT_Library library, FT_UInt max_faces, FT_UInt max_sizes, FT_ULong max_bytes, FTC_Face_Requester requester, 
